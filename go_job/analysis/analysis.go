...
func main() {
	// 获取参数
	logFilePath := flag.String("logFilePath", "/home/teng", "log file path")
...
func pvCounter(pvChannel chan urlData, storageChanel chan storageBlock)  {
...
func uvCounter(uvChannel chan urlData, storageChanel chan storageBlock, redisPool *pool.Pool)  {
...
func readFileLinebyLine(params cmdParams, logChannel chan string) error{
	...
	for {
		...
		if err != nil{
			if err == io.EOF{
				time.Sleep(3 *time.Second)
				log.Infof("ReadFileLinebyLine wait, readline: %s", count)
			}else {
				log.Warningf("ReadFileLinebyLine line: %d", count)
			}
		}
	}
}
...
func formatUrl(url, t string) urlNode {
	pos1 := str.IndexOf(url, HANDLE_HTML, 0)
	if pos1 != -1{
